...
Consider the following hypothetical scenario: a colleague from the biology department has just conducted an experiment and approaches you for statistical advice.%
%
%Suggestion Lexy:
%You are being told that the experiment is set up to test the null hypothesis that "plants grow with the same rate regardless if you talk to them or not". In other words, your colleague knows that his field still has considerable doubts about the working hypothesis "plants grow faster when you talk to them", which is why it has not yet been adopted in his field. 
%
%In line with recommendations both old (REFS) and new (REFS), your colleague doesn't only want to show that the effect exists, but also provide an estimate of the size of the effect. 
%
%There are two ways to do so: (1) conditionally on the assumption that the alternative is true, or (2) unconditionally by marginalising across both the null and the alternative hypothesis. 
%
%The conditional approach is standard. It is typically employed in a stepwise manner by first adopting the alternative that the effect is not zero after observing,  say, p < 0.05. An estimate of the effect size and a confidence interval is then given. Alternatively, the test is abandoned all together, because it is believed to be highly improbable anyways, and only the estimate and confidence interval is reported. 
%
%[Illustrate with example]
%
%The resulting effect size estimate in the stepwise conditional procedure of the effect size, can be too large if a mistake is made in the testing stage. Note that (tautologically) the best guess of the effect size when the null is true should be zero. However, even if the null hypothesis is true, then it is expected that p < 0.05 occurs in 5% of the experiments. As such, an estimate that should be zero is then replaced by an effect size estimate that was unlikely under the null (thus, far away from zero). 
%
%The unconditional or marginal approach elaborated on here attempts to correct for inflation of effect sizes in the stepwise approach, by acknowledging uncertainty in the testing phase.
%
...
\begin{figure}[!ht]
	\centering
	\includegraphics[width=\textwidth]{robustnessReanalysis.pdf}
	\caption{%
...
\end{revision}
\DON{call this adaptive shrinkage}

\section*{Discussion}
Standard estimates of effect size ignore the null hypothesis and are therefore overconfident, that is, \begin{revision}farther from
\end{revision} from zero \begin{revision}than they should be\end{revision}. The spike-and-slab model remedies this problem by explicitly considering the possibility that an effect is absent \parencite{Robinson2019,RouderEtAl2018PBR}. The core idea dates back to
...
The statistical counterargument is that, even if point null hypotheses are false, they are still mathematically convenient approximations to more complex hypotheses that allow mass on an interval close to zero \parencite[i.e., perinull hypotheses;][]{ly2020bayesian2, george1993variable, BergerDelampady1987}. 
...
%By reparametrizing into effect size we obtain $\delta = \nicefrac{\mu}{\sigma}$. The spike assumes that the effect size is 0, $\delta = 0$, while the slab introduces a prior distribution to represent prior uncertainty about possible effect sizes, $\delta \sim \dnorm{0}{\sigma_0^2}$.
%Thus, the likelihood $\lik$ under the slab is given by 
%\begin{align*}
%	\lik 	&= \prod_{i=1}^{N} \dnormc{Y_i}{\delta}{\sigma^2}\\
%			&= \prod_{i=1}^{N} \frac{1}{\sigma\sqrt{2\pi}} \exp\left(-\frac{1}{2\sigma^2}\left(Y_i - \delta\right)^2\right)\\
%%			&= \left(\frac{1}{\sigma\sqrt{2\pi}}\right)^N \exp\left(\frac{1}{2\sigma^2}\sum_{i=1}^{N}\left(Y_i - \delta\right)^2\right)\\
%			&= \left(\frac{1}{\sigma\sqrt{2\pi}}\right)^N \exp\left(-\frac{1}{2\sigma^2}
%			\left((n-1)s^2 +n(\mu - \bar{Y})\right)\right)
%\end{align*}
%where $\bar{Y}$ and $s$ are the mean and standard deviation of $Y$ respectively.  Following \textcite{RouderEtAl2018PBR}, we assume that $\sigma$ is known and reparametrize $\delta = \nicefrac{\mu}{\sigma}$ and $d = \nicefrac{\bar{Y}}{\sigma}$. This yields:
%\begin{align*}
%\lik 	&= \left(\frac{1}{\sigma\sqrt{2\pi}}\right)^N 
%\exp\left(\frac{1}{2\sigma} \left((n-1)s^2 +n(\delta - d)\right)\right).
%\end{align*}
%To obtain the likelihood under the slab, simply set $\delta=0$.
%
%The posterior distribution is obtained through Bayes' Theorem:
%\begin{align}%\label{eq:BayesTheorem2}
%\overbrace{\probp{\bm{\beta}, \bm{\model} \mid \data}}^{\substack{\text{Joint posterior}\\\text{distribution}}}
%= 
%\overbrace{\probp{\bm{\model}}}^{\substack{\text{Prior model}\\ \text{probability}}}
%\times	\quad
%\overbrace{\probp{\bm{\beta}\mid \bm{\model}}}^{\substack{\text{Prior param.}\\ \text{probability}}}
%\times \quad
%\frac{
%	\overbrace{\lik(\data \mid \bm{\beta}, \bm{\model})}^{\text{Likelihood}}
%}{
%%	\probp{\data}
%	\underbrace{\probp{\data}}_{\substack{\text{Marginal}\\ \text{Likelihood}}}
%}.
%\end{align}
%In practice, this is first done for each model separately and afterward the results are combined, although we will see that these steps can be combined for in example. To obtain the posterior distribution under the slab we have

%\probp{\bm{\beta}, \bm{\model} \mid \data}= 
%\frac{
%\prob{}
%
%	\overbrace{\lik(\data \mid \bm{\beta}, \bm{\model})}^{\text{Likelihood}}
